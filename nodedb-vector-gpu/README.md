# `nodedb-vector-gpu`

> GPU vector search backend (CUDA / cuVS / CAGRA)

Optional GPU implementation of the `nodedb-vector` index trait. Skeleton for cuVS / CAGRA integration; nvcc build is feature-gated and not enabled by default.

## Status

Pre-1.0. APIs may change between minor versions until 1.0. See the
workspace [`README.md`](../README.md) for the full project overview and
the GitHub [release notes](https://github.com/NodeDB-Lab/nodedb/releases)
for per-version changes.

## License

Licensed under the Apache License, Version 2.0 ([`LICENSE-APACHE`](../LICENSE-APACHE)).
